...
fn analyzes_all_languages_in_one_run() {
    ...
    assert_eq!(v["summary"]["file_count"], 13);
...
fn exclude_lang_drops_a_language() {
    ...
    let v = json(&[
        "--exclude-lang",
        "go,php,ruby,scheme,commonlisp,emacslisp,clojure,kotlin,python,zig,c",
        "tests/fixtures",
    ]);
...
fn excluding_every_language_is_an_error() {
    Command::cargo_bin("cccc")
        ...
        .args([
            "--exclude-lang",
            "es,rust,go,php,ruby,scheme,commonlisp,emacslisp,clojure,kotlin,python,zig,c",
            "tests/fixtures",
        ])
        .assert()
        .failure()
