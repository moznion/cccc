//! Clojure adapter: reads source with [lispexp](https://docs.rs/lispexp) (via
//! the shared [`cccc_lisp_kit`]) and lowers the S-expression datum tree into the
//! language-agnostic [`cccc_core::ir`](https://docs.rs/cccc-core).
//!
...
//! folding live in [`cccc_lisp_kit`]; this module supplies only the Clojure
//! reader preset ([`Options::clojure`]) and the special-form dispatch
//! ([`lower_list`]).
//!
...
pub fn analyze_source(path: &Path, source: &str) -> FileReport {
    cccc_lisp_kit::analyze(&Options::clojure(), lower_list, logical_op, path, source)
}
...
pub fn to_ir(_path: &Path, source: &str) -> (Vec<Node>, Vec<String>) {
    cccc_lisp_kit::lower(&Options::clojure(), lower_list, logical_op, source)
}
